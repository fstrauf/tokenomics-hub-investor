...
const postFields = `
  ...
  'comments': *[
    ...
    eli5,
  'author': author->{name, 'picture': image.asset->url, 'slug': slug.current},
`
...
export async function getAuthorAndPostsBySlug(slug, preview     ) {
  ...
  const [author, authorPosts] = await Promise.all([
    curClient
      ...
      .then((res) => res?.[0]),
    curClient.fetch(      
      `*[_type == "protocols" && author._ref in *[_type=="author" && slug.current==$slug]._id]{      
        ${postFields}
        body,
      }[0...4]`,
      { slug }
  ...
  return { author, authorPosts: getUniquePosts(authorPosts) }
}
...
export async function getAllAuthorsWithSlug() {
  const data = await client.fetch(`*[_type == "author"]{ 'slug': slug.current }`)
  return data
}
