                ...
                <div className="text-blueGray-600 mb-2 mt-10">
                  {catCount?.map((c) => {
                    return (
                      ...
                      <span
                        key={c.cat}
                        className="mr-2 rounded-full bg-gray-200 px-3 py-1 text-sm font-bold text-gray-700 shadow-sm"
                      >
...
export async function getStaticPaths() {
  ...
  const users = clerkConvertJSON(
    await clerkClient.users.getUserList({ userId })
  )
