...
export default function PostHeader({ title, updatedAt, shortDescription, type, tags, tokenStrength, slug, ticker }) {
  ...
  return (
    <div key={title}>
      ...
      <div className="text-lg flex flex-auto gap-4">
        ...
        {tags && tags.map((tag) => (
          <p key={tag.value} className='px-3 py-1 text-sm rounded-full bg-gray-600 text-white'>{tag?.label}</p>
        ))}
      </div>
      <div className="text-lg flex flex-auto gap-4">
      ...
      <div className='mt-6 px-4 py-1.5 mb-2 w-28 text-white font-medium text-sm leading-tight uppercase rounded-lg shadow-md hover:shadow-lg focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg transition duration-150 ease-in-out flex items-center bg-[#1da1f2]'>
        ...
        <TwitterShareButton title={`Check out the tokenomics of ${title} on Tokenomics Hub`} url={`${WEBSITE_URL_BASE}/posts/${slug}`}>
          Share
        </TwitterShareButton>
      </div>
    </div>
